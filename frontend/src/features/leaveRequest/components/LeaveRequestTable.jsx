...
export const renderLeaveTypeIcon = (type) => {
  switch (type) {
    case 'Annual Leave':
      ...
      <table className="w-full text-left border-collapse min-w-[760px]">
        <thead>
          <tr className="border-b border-slate-100 dark:border-slate-800/80 text-xs font-semibold text-slate-500 dark:text-slate-400">
            ...
            <th className="py-3.5 font-semibold">From</th>
            <th className="py-3.5 font-semibold">To</th>
        ...
        <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50 text-xs">
          {requests.map((req) => {
            ...
            const isRowSelected = selectedRequestId === req.id;
            const statusClass = statusBadgeClasses[req.status] || statusBadgeClasses.Pending;
            ...
            return (
              ...
              >
                {/* Checkbox */}
                <td
                  className="py-3.5 pl-2 whitespace-nowrap"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      <span
                        onClick={(e) => {
                          e.stopPropagation();
                          navigate(`/employees/${req.employeeId}`);
                        }}
                        className="font-bold text-slate-900 dark:text-white leading-tight hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"
                      >
                        {req.employeeName}
                      </span>
                ...
                {/* Reason */}
                <td className="py-3.5 pr-4 text-slate-600 dark:text-slate-400 max-w-[150px] truncate whitespace-nowrap">
                  {req.reason}
                </td>
