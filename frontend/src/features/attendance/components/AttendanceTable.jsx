...
const statusBadgeClasses = {
  ...
  'Half Day':
    'bg-sky-50 text-sky-600 dark:bg-sky-950/60 dark:text-sky-400 border-sky-100/90 dark:border-sky-800/60',
};
    ...
    return (
      <div className="py-16 text-center">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
          {isHr
            ? 'No employees match this date or filter.'
            : 'Clock in to create today’s attendance, or pick another date.'}
        </p>
      </div>
    );
  }
      ...
      <table className="w-full text-left border-collapse min-w-[760px]">
        ...
        <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50 text-xs">
          {records.map((rec) => {
            ...
            const avatarTheme = rec.avatarTheme || 'purple';
            const initials =
              rec.initials ||
              (rec.employeeName || '')
                .split(' ')
                ...
                .join('')
                .slice(0, 2);
            const profileId = rec.employeeUuid || rec.employeeId;
            ...
            return (
              ...
              >
                ...
                <td className="py-3 pr-4 whitespace-nowrap">
                  <div
                    onClick={() => navigate(`/employees/${profileId}`)}
